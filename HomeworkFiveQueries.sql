...
GO 
CREATE FUNCTION dbo.rooms_by_price (@min int, @max int)
RETURNS TABLE
AS
RETURN (
...
-- QUESTION 7	doesn't work, kind of there but ran out of time
INSERT INTO Rooms
	SELECT 1 id FROM Taverns AS tavern_id,
	(SELECT 1 id FROM Status) AS status_id,
	(SELECT Rooms.costs - 0.01 FROM Rooms WHERE Rooms.costs = (SELECT min(Price) FROM dbo.rooms_by_price(0,1000)));